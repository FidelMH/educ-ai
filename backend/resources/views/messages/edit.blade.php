<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier le message #' . $message->id) }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('dashboard.messages.update', $message) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input
                                ...
                                name="type_message"
                                id="type_message"
                                value="{{ old('type_message', $message->type_message) }}"
                        ...
                        <div class="mb-4">
                            ...
                            >
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $message->user_id) == $user->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            >
                                ...
                                @foreach($discusses as $discuss)
                                    <option value="{{ $discuss->id }}" {{ old('discuss_id', $message->discuss_id) == $discuss->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            >
                                ...
                                @foreach($agents as $agent)
                                    <option value="{{ $agent->id }}" {{ old('agent_id', $message->agent_id) == $agent->id ? 'selected' : '' }}>
                                        Agent #{{ $agent->id }}
                                    </option>
                                @endforeach
                            </select>
